...
class TelaDoacao:
    ...
    def pega_dados_doacao(self):
        ...
        layout = [
            [
                sg.Text(
                    "Data da doação (Exemplo: 31/12/1999):",
                    size=(width_size, height_size),
                ),
                sg.InputText("", key="data_doacao"),
            ],
            [
                sg.Text("CPF do doador:", size=(width_size, height_size)),
                sg.InputText("", key="cpf_doador"),
            ],
            [
                sg.Text("Motivo da doação:", size=(width_size, height_size)),
                sg.InputText("", key="motivo"),
            ],
            [sg.Push(), sg.Button("Confirmar"), sg.Cancel("Cancelar")],
        ]
    ...
    def mostra_doacao(self, dados_doacoes: list, mensagem: str = "Lista de doações:"):
        ...
        for doacao in dados_doacoes:
            ...
            string_todas_doacoes += (
                f"Data da doação: {doacao['data_doacao'].strftime('%d/%m/%Y')}\n"
            )
    ...
    def seleciona_doacao(self):
        layout = [
            [
                sg.Text("Código da doação que deseja selecionar: "),
    ...
    def pega_dados_doacao_altera(self):
        ...
        layout = [
            ...
            [
                sg.Text("Nome do doador:", size=(width_size, height_size)),
                sg.InputText("", key="nome_doador"),
            ],
            [
                sg.Text("Motivo da doação:", size=(width_size, height_size)),
                sg.InputText("", key="motivo"),
