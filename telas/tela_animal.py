...
class TelaAnimal:
    ...
    def pega_dados_animal(self):
        ...
        layout = [
            ...
            [
                sg.Text("Tipo:", size=(width_size, height_size)),
                sg.Radio("Cachorro", "Radio1", default=True, key="cachorro"),
                sg.Radio("Gato", "Radio1", key="gato"),
            ],
            [
                sg.Text("Raça:", size=(width_size, height_size)),
                sg.InputText("", key="raca"),
            ],
            [
                sg.Text("Tamanho (se for cachorro):", size=(width_size, height_size)),
                sg.Combo(("Pequeno", "Médio", "Grande"), readonly=True, key="tamanho"),
            ],
            [sg.Push(), sg.Button("Confirmar"), sg.Cancel("Cancelar")],
        ]
    ...
    def mostra_animal(self, dados_animais: list):
        ...
        for animal in dados_animais:
            ...
            if animal["pode_ser_adotado"] and not animal["foi_adotado"]:
                ...
                string_todos_animais += (
                    "   Disponível para adoção: Não, pois já foi adotado.\n\n"
                )
            else:
