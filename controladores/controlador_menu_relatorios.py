...
class ControladorMenuRelatorios:
    ...
    def adocoes_por_periodo(self):
        ...
        if contador_adocoes == 0:
            ...
            self.__controlador_sistema.controlador_adocao.tela_adocao.mostra_adocao(
                dados_adocoes,
                f"Lista de adoções entre:\n{periodo['inicial'].strftime('%d/%m/%Y')} e {periodo['final'].strftime('%d/%m/%Y')}",
            )
    ...
    def doacoes_por_periodo(self):
        ...
        if contador_doacoes == 0:
            ...
            self.__controlador_sistema.controlador_doacao.tela_doacao.mostra_doacao(
                dados_doacoes,
                f"Lista de doações entre {periodo['inicial'].strftime('%d/%m/%Y')} e {periodo['final'].strftime('%d/%m/%Y')}",
            )
